...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.canvas.browseRequest.connect(self.browseRequest)
        ...
        #Slider widget for adjust frame number
        self.total_frames = self.fileListWidget.count()
        ...
        self.frameSlider.setRange(0, self.total_frames - 1)
        ...
        # 将滑动条添加到底部
        self.slider_dock = QtWidgets.QDockWidget(self.tr("Frame Slider"), self)
        self.slider_dock.setObjectName("Slider")
        self.slider_dock.setWidget(self.frameSlider)
        self.addDockWidget(Qt.BottomDockWidgetArea, self.slider_dock)
        self.flag_slider = True
    ...
    def frameSliderChanged(self, idx):
        self.fileListWidget.setCurrentRow(idx)
        tar_path = self.imageList[self.fileListWidget.currentRow()]
        assert tar_path in self.imageList, f"{tar_path} not in {self.imageList}"
        self.loadFile(tar_path)
        print("frameSliderChanged", idx)
        print("Load:", tar_path)
        # self.fileListWidget.setCurrentRow(self.fileListWidget.currentRow() + action)
        # tar_path = self.imageList[self.fileListWidget.currentRow()]
        # assert tar_path in self.imageList,f"{tar_path} not in {self.imageList}"
        # self.loadFile(tar_path)
        # print('Loaded:', tar_path)
        # print("frameSliderChanged", value)
    ...
    def browseRequest(self, action:int):
        act = 'next' if action == 1 else 'previous'
        ic('Received action:', act)
        #-1: previous, 1: next
        self.fileListWidget.setCurrentRow(self.fileListWidget.currentRow() + action)
        tar_path = self.imageList[self.fileListWidget.currentRow()]
        assert tar_path in self.imageList,f"{tar_path} not in {self.imageList}"

        self.frameSlider.setValue(self.fileListWidget.currentRow())
